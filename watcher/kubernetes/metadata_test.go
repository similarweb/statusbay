...
func TestGetMetricsDataFromAnnotations(t *testing.T) {
	...
	annotations := map[string]string{
		fmt.Sprintf("%s/metrics-providername-metric-name", annotationPrefix): "metric query 1",
	}
...
func TestGetAlertsDataFromAnnotations(t *testing.T) {
	annotations := map[string]string{
		fmt.Sprintf("%s/alerts-providername", annotationPrefix): "foo,foo1",
	}
...
func TestGetProgressDeadlineApply(t *testing.T) {
	...
	t.Run("get_progress_deadline_annotation", func(t *testing.T) {
		...
		annotations := map[string]string{
			fmt.Sprintf("%s/%s", annotationPrefix, annotationProgressDeadlineSeconds): "10",
		}
		progressDeadlineSeconds := GetProgressDeadlineApply(annotations, 2)

		if progressDeadlineSeconds != 10 {
			t.Fatalf("unexpected %s annotation value, got %d expected %d", annotationProgressDeadlineSeconds, progressDeadlineSeconds, 10)
	...
	t.Run("get_un_exists_progress_deadline_annotation", func(t *testing.T) {
		...
		if progressDeadlineSeconds != 60 {
			t.Fatalf("unexpected %s annotation value, got %d expected %d", annotationProgressDeadlineSeconds, progressDeadlineSeconds, 60)
...
func TestGetApplicationName(t *testing.T) {
	...
	t.Run("get_name_annotation", func(t *testing.T) {
		...
		annotations := map[string]string{
			fmt.Sprintf("%s/%s", annotationPrefix, annotationApplicationName): "foo",
		}
		applicationName := GetApplicationName(annotations, "default")

		if applicationName != "foo" {
			t.Fatalf("unexpected %s annotation value, got %s expected %s", annotationApplicationName, applicationName, "foo")
	...
	t.Run("get_un_exists_progress_deadline_annotation", func(t *testing.T) {
		...
		if applicationName != "default" {
			t.Fatalf("unexpected %s annotation value, got %s expected %s", annotationApplicationName, applicationName, "default")
